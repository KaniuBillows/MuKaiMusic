...
{
    public class PlaylistService : BaseRemoteService
    {

        private readonly IMongoCollection<UserMusic> userMusicCollection;

        public PlaylistService(IHttpClientFactory httpClientFactory, IMongoDatabase database, IConfiguration configuration)
            : base(httpClientFactory, configuration)
        {
            this.userMusicCollection = database.GetCollection<UserMusic>("user_musics");
        }


        public async Task<string> GetPlaylistDetail(long id)
        {
            return await this.ServiceRequest(DataSource.NetEase, $"/detail/playlist?id={id}");
        }

        public async Task<string> GetPersonalizedPlaylist(int limit)
        {
            return await this.ServiceRequest(DataSource.NetEase, $"/personlized/playlist?limit={limit}");
        }
